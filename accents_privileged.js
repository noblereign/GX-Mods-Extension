...
function hexCoercion(colorString) { // converts colors TWICE to make sure that they end up as a hex string. im so sorry for this color crime
...
browser.theme.getCurrent().then(async function(currentTheme) {
    ...
    injectAccents(accentColor, backgroundColor)
})
.catch(async function(err) {
    console.warn(`[GXM] Failed to get theme colors! ${err}`)
    const accentColor = '#fa1e4e';
    const backgroundColor = (await getPreferredColorScheme() == "dark" ? '#121019' : '#f4f2f7');
    injectAccents(accentColor, backgroundColor)
});  

function handleUpdated(updateInfo) {
    let accentColor = updateInfo.theme.colors?.icons_attention ?? '#fa1e4e';
    let backgroundColor = updateInfo.theme.colors?.toolbar ?? '#121019';
